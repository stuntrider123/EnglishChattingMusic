...
async def pause_cmd(_, message): 
    ...
    if not await is_active_chat(chat_id):
        return await message.reply_text("**not play anything!**")
    elif not await is_music_playing(message.chat.id):
        return await message.reply_text("**not play anything!**")   
    await music_off(chat_id)
    await music.pytgcalls.pause_stream(chat_id)
    await message.reply_text(f"**🎶music paused by** {checking}")
...
async def stop_cmd(_, message): 
    ...
    if not await is_active_chat(chat_id):
        ...
        await message.reply_text(f"**🎶music resumed...**")
...
async def stop_cmd(_, message): 
    ...
    if await is_active_chat(chat_id):
        ...
        await remove_active_chat(chat_id)
        await music.pytgcalls.leave_group_call(chat_id)
        await message.reply_text(f"**music ended by** {checking}!") 
    else:
        return await message.reply_text("not play anything to end!")
...
async def stop_cmd(_, message): 
    ...
    if not await is_active_chat(chat_id):
        await message.reply_text("**no queue to skip**")
    else:
        task_done(chat_id)
        if is_empty(chat_id):
            await remove_active_chat(chat_id)
            await message.reply_text("no any music in queue, userbot leave the vc.")
            await music.pytgcalls.leave_group_call(chat_id)
            return  
        else:
            ...
            if str(finxx) != "raw":   
                ...
                reply_markup=InlineKeyboardMarkup(buttons),
                caption=f"<b>**Skipped music, now playing...**</b>\n\n〃<b>**Name:**</b> {title} \n〃<b>**Duration:**</b> {duration} \n〃<b>**Request by:** </b> {username}",
                )
                return
