...
async def mpthree(_, message: Message):
    ...
    if message.sender_chat:
        return await message.reply_text(
            """
Anonymous admin.
Get back to user account with admin rights.
"""
        )
    ...
    if url:
        ...
        m = await message.reply_text(
            f"""
<b>彡 Judul:</b> [{title[:25]}]({url})
<b>彡</b> [More Information](https://t.me/{BOT_USERNAME}?start=info_{id})
<b>彡 Didukung</b> [{BOT_NAME}](t.me/{BOT_USERNAME})
""",
...
"""
        ...
        hmo = await message.reply_text(
            f"""
<b>⬇️ MUSIC/VIDEO DOWNLOADER ⬇️</b>


¹ <b>{title1[:20]}</b>
├ 💡 [More Information](https://t.me/{BOT_USERNAME}?start=info_{ID1})
└ ⚡ **Search by:** [{BOT_NAME}](t.me/{BOT_USERNAME})
...
async def startyuplay(_, CallbackQuery):
    ...
    m = await CallbackQuery.message.reply_photo(
        ...
        reply_markup=InlineKeyboardMarkup(buttons),
        caption=f"""
<b>彡 Name:</b> [{title[:25]}]({url})
├ 彡 [More Information](https://t.me/{BOT_USERNAME}?start=info_{id})
└ 彡 **Search by:** [{BOT_NAME}](t.me/{BOT_USERNAME})
""",
    )
    os.remove(thumb)
    await CallbackQuery.message.delete()
